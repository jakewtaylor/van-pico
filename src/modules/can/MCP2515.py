...
class MCP2515():
    def __init__(
        ...
        self.spi = SPI(spi_id)
        self.spi = SPI(spi_id, baudrate, polarity=polarity, phase=phase, sck=sck, mosi=mosi, miso=miso)
    ...
    def WriteByte(self, addr: int):
        self.cs(0)
        self.spi.write(bytearray([addr]))
        self.cs(1)

    def WriteBytes(self, addr: int, data: int):
        self.cs(0)
    ...
    def Send(self, CAN_ID: int, CAN_TX_Buf: list[int], length1: int):
